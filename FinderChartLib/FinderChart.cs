...
namespace FinderChartLib
{
    public class FinderChart
    {
		...
		public void SetFieldView(int imageWidth, float limitingMagnitude, float radiusRadians, float raRadians, float declinationRadians, string title)
		{
            ...
            this._legendFont = new Font(FontFamily.GenericSerif, 10 * this._scale);
		...
		private void DrawStars(Graphics g)
		{
			for(int i = 0; i < this.StarCatalog.Stars.Length; i++)
			{
				if (this.StarCatalog.Stars[i].Magnitude <= this._limitingMagnitude) {
					...
					if (angularDistance < this._radiusScale)
					{
		...
		private void DrawMagnitudeScale(Graphics g)
		{
			...
			{
				DrawStar(g, 20, this._imageWidth - yOffset + 10, i);
				g.DrawString(i.ToString(), this._legendFont, this._foregroundBrush, 30, this._imageWidth - yOffset);
				yOffset += 20;
			}
		}
	}
